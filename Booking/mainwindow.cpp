    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->enterTelephoneForAuthorizationLineEdit->setValidator(
        new QRegularExpressionValidator(telephoneRegExpr));
    connect(ui->pushButton, &QPushButton::clicked, [this]
        {
            ...
            const size_t hashedPasswordAgain
                = h(ui->enterPasswordAgainLineEdit->text().toUtf8().constData());
                ...
                && ui->isPasswordOkLabel->text().toStdString() == "Good")
            {
                try
                {
                    const std::string idB
                        = db::ClientAPI::createClient(telephone, password, name, email);
    ...
    connect(ui->enterButton, &QPushButton::clicked, [this]
        {
            std::hash<std::string> h;
            const size_t hashedPassword
                = h(ui->enterPasswordForAuthorizationLineEdit->text().toUtf8().constData());
            std::string password1 = std::to_string(hashedPassword);
            std::string telephone1
                = ui->enterTelephoneForAuthorizationLineEdit->text().toUtf8().constData();
            try
            {
